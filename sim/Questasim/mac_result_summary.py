...
def uvm_count(text: str, severity: str) -> int:
    match = re.search(rf"^#\s*UVM_{severity}\s*:\s*(\d+)", text, re.MULTILINE)
...
def capture(args: argparse.Namespace) -> int:
    ...
    # Questa assertions and elaboration errors are not necessarily UVM
    # reports and can still leave a zero process exit status. Count them so a
    # batch regression cannot incorrectly advertise a simulator error as PASS.
    simulator_errors = len(re.findall(r"^#\s*\*\* Error(?:\s*\([^)]*\))?:", text,
                                      re.MULTILINE))
    simulator_errors += len(re.findall(r"^#\s*Error loading design\s*$", text,
                                       re.MULTILINE))
    total_errors = errors + fatals + simulator_errors
